...
out$bonferroni = p.adjust(out$p, method = "bonferroni")
...
ggplot(out, aes(x = Dif_med, y = -log(p), label = paste(Gene," (n=", nfg,")",
  ...
  ggrepel::geom_text_repel(data = filter(out, p<=0.005), size = 2.5, family = "Arial", segment.size = 0.1,
                           nudge_y = 1, min.segment.length = 0.01, col = "darkblue")+
  geom_text(data = filter(out, p>0.005), aes(label = Gene), size = 1.5, family = "Arial", col = "grey50",
            nudge_y = -0.3, col = "lightblue")+
  geom_hline(yintercept = -log(0.005), lty = 2, col = "grey50", size = 0.2)+
  annotate(geom = "text", x = -80, y = -log(0.005)+0.1, label = "p = 0.005", 
  ...
  #scale_x_log10()+
  scale_x_continuous(trans = pseudolog10_trans, breaks = c(-1e2,0,1e3,5e3, 1e4))+
...
#ggsave("Figures/Fig4//volcano.pdf", width = 7, height = 4, device = "pdf")
